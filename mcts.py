...
class MonteCarlo(StdOutPlayer):
    ...
    def pick_random_sub_board_coords(sub_board: SubBoard) -> SubBoardCoords: #this means: argument should be of type 'SubBoard' and
        node = Node(board_state=SubBoard) #do we need to pass legal moves now so we can make the kids?)
...
class Node(object):
    ...
    def monte_carlo_tree_search(self):
        ...
        while datetime.datetime.utcnow() - begin < datetime.timedelta(milliseconds=4.9):
            ...
            if all(child.visited for child in self.children):
            #next node to explore is one with max UCT
                return max_uct(self)   #this returns the best poss next move
            #if some unvisited:
            else:
                self.run_simulation(node)
        return max_uct(self)

    def run_simulation(self):
        if MainBoard.is_finished: ###
            self.plays += 1 #shouldn't be self.?
            if MainBoard.winner == 1:
                ...
                return self.plays #just return result? don't need three if statements
        else:
            next_child = random.choice(self.children)
            self.visited = True
            self.wins = wins + run_simulation(next_child)
            #make this wins, play = ?

    def max_uct(node, c=1.4):
        uct_vals = [
            (child.wins / (child.total_plays)) + c_param * np.sqrt((2 * np.log(self.n) / (c.n)))
            for child in node.children
        ] #first terms = ratio wins of child, second term is total ratio wins
        return self.children[np.argmax(uct_vals)]
